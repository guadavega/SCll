...
B = [0; 1/M; 0; -1/(M*l)];
C = [1 0 0 0; 0 0 1 0];  % Sólo se mide el desplazamiento y el ángulo
D = [0; 0];
...
x = [0; 0; pi; 0];
x_hat = [0; 0; 0; 0]; % Estado inicial del observador
...
for i = 1:length(t)
    ...
    if m2_changed
        dx_hat = Am2 * x_hat + B * u + L2 * (C * x - C * x_hat);
    else
        dx_hat = A * x_hat + B * u + L * (C * x - C * x_hat);
    end
    x_hat = x_hat + dx_hat * delta_T;
...
figure;
subplot(3, 1, 1);
plot(t, x_hist(:, 1), 'b', t, x_hat_hist(:, 1), 'r--');
...
subplot(3, 1, 2);
plot(t, x_hist(:, 3), 'b', t, x_hat_hist(:, 3), 'r--');
title('Ángulo \phi');
xlabel('Tiempo (s)');
ylabel('Ángulo (rad)');
legend({'Real', 'Observado'});
